...
module.exports = ({ files, markdownNode, markdownAST, pathPrefix, getNode, reporter }, pluginOptions) => {
  ...
  <span
    ...
    style="position: relative; display: block; ${options.wrapperStyle}; max-width: ${presentationWidth}px; margin-left: auto; margin-right: auto; flex: ${responsiveSizesResult.aspectRatio}"
  >
    <span
  ...
  <a
    ...
    href="${originalImg}"
    style="display: block; flex: ${responsiveSizesResult.aspectRatio}"
    target="_blank"
    rel="noopener"
  >
  ${rawHTML}
    ...
    if (options.showCaptions && node.title) {
      rawHTML = `
  <figure class="gatsby-resp-image-figure" style="flex:${responsiveSizesResult.aspectRatio}">
  ${rawHTML}
